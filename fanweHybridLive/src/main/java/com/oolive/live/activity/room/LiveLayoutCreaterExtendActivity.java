...
import com.oolive.auction.dialog.AuctionCreateAuctionDialog;
...
import com.oolive.o2o.dialog.O2OShoppingPodCastDialog;
import com.oolive.pay.LiveScenePayCreaterBusiness;
import com.oolive.pay.LiveTimePayCreaterBusiness;
import com.oolive.pay.dialog.LiveImportPriceDialog;
import com.oolive.pay.dialog.LiveScenePriceDialog;
import com.oolive.pay.model.App_live_live_payActModel;
import com.oolive.pay.model.App_monitorLiveModel;
import com.oolive.pay.room.RoomLivePayInfoCreaterView;
import com.oolive.pay.room.RoomLiveScenePayInfoView;
import com.oolive.shop.dialog.ShopMyStoreDialog;
import com.oolive.shop.dialog.ShopPodcastGoodsDialog;
...
public class LiveLayoutCreaterExtendActivity extends LiveLayoutCreaterActivity implements LiveTimePayCreaterBusiness.LiveTimePayCreaterBusinessListener, LiveScenePayCreaterBusiness.LiveScenePayCreaterBusinessListener {
    ...
    protected void onClickCreaterPluginNormal(PluginModel model) {
        ...
        if (LiveConstant.PluginClassName.P_PAI.equalsIgnoreCase(model.getClass_name())) {
            clickPai();
        } else if (LiveConstant.PluginClassName.P_SHOP.equalsIgnoreCase(model.getClass_name())) {
            clickShop();
        } else if (LiveConstant.PluginClassName.P_PODCAST_GOODS.equalsIgnoreCase(model.getClass_name())) {
            clickPodcast_goods();
        } else if (LiveConstant.PluginClassName.P_LIVE_PAY.equalsIgnoreCase(model.getClass_name())) {
            clickPayMode(model);
        } else if (LiveConstant.PluginClassName.P_LIVE_PAY_SCENE.equalsIgnoreCase(model.getClass_name())) {
            clickPayScene();
        }
    }
