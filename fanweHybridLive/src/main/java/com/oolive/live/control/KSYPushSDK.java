package com.oolive.live.control;
...
import com.oolive.live.model.LiveKSYBeautyConfig;
...
public class KSYPushSDK implements IPushSDK, SDHeadsetPlugReceiver.HeadsetPlugCallback {
    ...
    public void setMirror(boolean mirror) {
        mIsMirror = mirror;
        if (mPusher.isFrontCamera()) {
            mPusher.setFrontCameraMirror(mIsMirror);
        }
    }
    ...
    private void restoreMicVolume() {
        if (mIsBGMStarted) {
            MusicEffectConfig config = SDDisk.open().getSerializable(MusicEffectConfig.class);
            if (config == null) {
                setMicVolume(100);
            } else {
                setMicVolume(config.getMicVolume());
            }
        } else {
            setMicVolume(100);
        }
