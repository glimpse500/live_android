...
import com.oolive.hybrid.dialog.BotPhotoPopupView;
import com.oolive.hybrid.dialog.DialogCropPhoto;
import com.oolive.hybrid.dialog.DialogCropPhoto.OnCropBitmapListner;
...
import com.oolive.hybrid.map.tencent.SDTencentGeoCode;
import com.oolive.hybrid.map.tencent.SDTencentGeoCode.Geo2addressListener;
...
import com.oolive.library.utils.UriFileUtils;
import com.oolive.library.webview.CustomWebView;
import com.oolive.library.webview.DefaultWebChromeClient;
import com.oolive.library.webview.DefaultWebViewClient;
import com.oolive.live.R;
import com.oolive.live.common.CommonInterface;
import com.oolive.live.event.EWxPayResultCodeComplete;
import com.oolive.live.model.App_userinfoActModel;
import com.oolive.live.wxapi.WXPayEntryActivity;
import com.oolive.o2o.event.EO2ORefreshH5HomePage;
import com.oolive.o2o.event.EO2OShoppingLiveMainExist;
import com.oolive.o2o.jshandler.O2OShoppingLiveJsHander;
...
public class MainActivity extends BaseActivity implements OnCropBitmapListner, PayResultListner {
        ...
        SDTencentMapManager.getInstance().startLocation(new TencentLocationListener() {
            ...
            public void onLocationChanged(TencentLocation location, int error, String reason) {
                if (location != null) {
                    ...
                    mWebViewCustom.loadJsFunction(Constant.JsFunctionName.JS_POSITION, lat, lng);
        ...
        mGeoCode.location(new LatLng(lat, lng)).geo2address(new Geo2addressListener() {
            ...
            public void onSuccess(ReverseAddressResult result) {
                if (result.formatted_addresses != null) {
                    ...
                    mWebViewCustom.loadJsFunction(Constant.JsFunctionName.JS_POSITION2, lat, lng, json);
                }
            }
    ...
    private void openShare(SdkShareModel model) {
        ...
        UmengSocialManager.openShare(title, content, imageUrl, clickUrl, this, new UMShareListener() {
            ...
            public void onResult(SHARE_MEDIA platform) {
                ...
                mWebViewCustom.loadJsFunction(Constant.JsFunctionName.SHARE_COMPLEATE, key);
            }
    ...
    public void callbackbitmap(Bitmap bit) {
        ...
        mWebViewCustom.loadJsFunction(Constant.JsFunctionName.CUTCALLBACK, base64img);
    }
    ...
    public void onActivityResult(int requestCode, final int resultCode, final Intent data) {
        ...
        switch (requestCode) {
            ...
            case REQUEST_CODE_UPAPP_SDK:
                ...
                break;
            case AppJsHandler.REQUEST_CODE_WEB_ACTIVITY:
            ...
            case AppJsHandler.REQUEST_CODE_QR:
                if (resultCode == MyCaptureActivity.RESULT_CODE_SCAN_SUCCESS) {
                    ...
                    mWebViewCustom.loadJsFunction(Constant.JsFunctionName.JS_QR_CODE_SCAN, str_qr);
                }
                break;
    ...
    private void judgeUrlBack(InitActModel model) {
        ...
        if (model.getTop_url() != null && model.getTop_url().size() > 0) {
            ...
            if (!isEqualsUrl) {
                mWebViewCustom.loadJsFunction(Constant.JsFunctionName.JS_BACK, new Object[]{});
            } else {
                isEqualsUrl = false;
            }
        } else {
    ...
    public void onSuccess() {
        mHandler.post(new Runnable() {
            @Override
            public void run() {
                mWebViewCustom.loadJsFunction(Constant.JsFunctionName.JS_PAY_SDK, 1);
            }
        });
    }
    ...
    public void onDealing() {
        mHandler.post(new Runnable() {
            @Override
            public void run() {
                mWebViewCustom.loadJsFunction(Constant.JsFunctionName.JS_PAY_SDK, 2);
            }
        });
    }
    ...
    public void onFail() {
        mHandler.post(new Runnable() {
            @Override
            public void run() {
                mWebViewCustom.loadJsFunction(Constant.JsFunctionName.JS_PAY_SDK, 3);
            }
        });
    }
    ...
    public void onCancel() {
        mHandler.post(new Runnable() {
            ...
            @Override
            public void run() {
                mWebViewCustom.loadJsFunction(Constant.JsFunctionName.JS_PAY_SDK, 4);
            }
        });
    }
    ...
    public void onNetWork() {
        mHandler.post(new Runnable() {
            @Override
            public void run() {
                mWebViewCustom.loadJsFunction(Constant.JsFunctionName.JS_PAY_SDK, 5);
            }
        });
    }
    ...
    public void onOther() {
        mHandler.post(new Runnable() {
            @Override
            public void run() {
                mWebViewCustom.loadJsFunction(Constant.JsFunctionName.JS_PAY_SDK, 6);
            }
        });
    }
    ...
    public void onEventMainThread(EIsExistInstalled event) {
        ...
        mWebViewCustom.loadJsFunction(Constant.JsFunctionName.JS_IS_EXIST_INSTALLED, is_exist_sdk, is_exist);
    }
    ...
    public void onEventMainThread(EApns event) {
        ...
        mWebViewCustom.loadJsFunction(Constant.JsFunctionName.JS_APNS, "android", dev_token);
    }

    public void onEventMainThread(ELoginSdk event) {
        String type = event.type;
        showProgressDialog("");
        if (Constant.LoginSdkType.WXLOGIN.equals(type)) {

        } else if (Constant.LoginSdkType.QQLOGIN.equals(type)) {
    ...
    public void onEventMainThread(EClipBoardText event) {
        ...
        mWebViewCustom.loadJsFunction(Constant.JsFunctionName.GET_CLIP_BOARD, text);
    }
