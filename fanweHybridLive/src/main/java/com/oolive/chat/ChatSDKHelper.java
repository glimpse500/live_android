...
public class ChatSDKHelper {
        ...
        msgListener = ChatSDK.events().sourceOnMain()
                ...
                .subscribe(new Consumer<NetworkEvent>() {
                    ...
                    public void accept(NetworkEvent networkEvent) throws Exception {
                        ...
                        try {
                            if (json_message.equals("EImOnForceOffline")){
                                LogUtil.i("EImOnForceOffline");
                                EImOnForceOffline event = new EImOnForceOffline();
    ...
    private static void registe_login(String raw_userID,Activity activity){
        ...
        Disposable d = ChatSDK.auth().authenticate(details).subscribe(() -> {
                    ...
                    ChatSDK.auth().authenticate(details).subscribe(() -> {
                        LogUtil.i("註冊私聊帳號成功 "+ ChatSDK.currentUserID() );
                        SDToast.showToast("註冊私聊帳號成功");
                        CommonInterface.updateChatID(null,ChatSDK.currentUser().getEntityID());
                        dismissProgressDialog();
                    }, throwable2 -> {
    ...
    public static void sendMsgC2C(final String id, final User peer, final CustomMsg customMsg, final SocketIOValueCallBack<SocketIOMessage> callback) {
            ...
            CommonInterface.requestIs_black(customMsg.getSender().getUser_id(), new AppRequestCallback<User_is_blackActModel>() {
                protected void onSuccess(SDResponse resp) {
                    if (actModel.isOk() && !actModel.getIs_black()) {
                        ...
                        user_list.add(peer);
                        Thread c2cThread = ChatSDK.db().fetchThreadWithUsers(user_list);
                        if (c2cThread == null){
                            ChatSDK.thread().createThread("c2cMsgThread", user_list)
                                    .observeOn(AndroidSchedulers.mainThread())
                                    .doFinally(() -> { })
                                    .subscribe(thread -> {
                                        ChatSDK.thread().sendMessageWithText(json, c2cThread).subscribe(progress -> {
                                            if (progress.getStatus() == MessageSendStatus.Sent){
                                                callback.onSuccess(customMsg.parsetoSocketIOMessage());
                                            }
                                        },throwable -> { LogUtil.i("sendMessageWithText error2  "+ throwable); });
                                    }, throwable -> {
                                        LogUtil.i("throwable  "+ throwable);
                                        SDToast.showToast("創建私聊異常" + throwable);
                                    });
                        }
                        else {
                            ChatSDK.thread().sendMessageWithText(json, c2cThread).subscribe(progress -> {
